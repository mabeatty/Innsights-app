...
// Revenue for one fee type, broken out by project, across the fixed
// 24-month Revenue calendar — actuals (real, from QuickBooks) plus a
// forecast section (not-yet-billed, from revenue_forecast). A revenue type
// with no forecast rows loaded (e.g. Consulting Fees, as of this writing)
// simply shows an empty forecast section rather than fabricating one.
export default function CompanyRevenueTab({ revenueType, title }: CompanyRevenueTabProps) {
  const { loading, error, monthlyTotals, projectTotals, total, forecastTotal } = useCompanyRevenue(revenueType);
  ...
  return (
    <div className="space-y-6 pt-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <KpiCard label="Total forecast" value={hasForecast ? fmtFull(forecastTotal) : "No forecast loaded"} sub="Not yet billed" />
      </div>
