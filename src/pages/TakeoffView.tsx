...
import { ArrowLeft, Download, Loader2, Plus, Save, Trash2, X } from "lucide-react";
...
export default function TakeoffView() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("room_matrix_entries").select("*").eq("project_id", project.id),
  ...
  const handleSaveMatrix = async () => {
    ...
    try {
      await supabase.from("room_matrix_entries").delete().eq("project_id", project.id);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const { data: versionData, error: vErr } = await supabase
        .from("takeoff_versions")
        .insert({ project_id: project.id, version_number: nextVersion })
        .select()
        .single();
  ...
  /* ── Inline editing ── */
  const updateTakeoffItem = async (itemId: string, field: "adjusted_quantity" | "notes", value: number | string | null) => {
    await supabase.from("takeoff_line_items").update({ [field]: value, last_modified: new Date().toISOString() }).eq("id", itemId);
    setTakeoffItems((prev) => prev.map((i) => (i.id === itemId ? { ...i, [field]: value } : i)));
  };

  const updatePublicItem = async (itemId: string, field: "adjusted_quantity" | "notes", value: number | string | null) => {
    await supabase.from("project_public_area_items").update({ [field]: value, last_modified: new Date().toISOString() }).eq("id", itemId);
    setPublicAreaItems((prev) => prev.map((i) => (i.id === itemId ? { ...i, [field]: value } : i)));
  };

  const displayQty = (qr: number, aq: number | null) => aq ?? qr;
  ...
  // De-duplicate if same item appears in both room and bathroom (unlikely but safe)
  ...
  const fileName = (ext: string) => `${project?.name ?? "takeoff"}-v${versions.find((v) => v.id === activeVersionId)?.version_number ?? ""}.${ext}`;
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(14);
    doc.text(`${project?.name ?? "Takeoff"} - v${versions.find((v) => v.id === activeVersionId)?.version_number ?? ""}`, 14, 15);
    autoTable(doc, {
      head: [headers],
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-foreground">FF&E Takeoff</h1>
      ...
      {activeVersion && hasTakeoff && (
        <div className="space-y-6">
          ...
          {/* ── Guestroom FF&E (aggregated by item, grouped by category) ── */}
          ...
          {/* ── Public Area FF&E (aggregated by item, grouped by category) ── */}
